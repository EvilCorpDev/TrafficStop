...
public class TrafficWidget extends AppWidgetProvider {
	...
	public void onReceive(Context context, Intent intent) {
		...
		if(action.equals(TrafficControl.BROADCAST_ACTION)){
			...
			for (int appWidgetID : ids) {
				updateText(context, appWidgetManager, appWidgetID, intent.getDoubleExtra("traff", 0), 
						intent.getDoubleExtra("allTraff", 0), intent.getDoubleExtra("mBytes", 0));
			}
		}
	...
	public void onUpdate(Context context, AppWidgetManager appWidgetManager,
		...
		for(int id : appWidgetIds){
			updateText(context, appWidgetManager, id, hm2.get("end"), hm2.get("end"), mBytes);
		}
	}
		
	public void updateText(Context context, AppWidgetManager appWidgetManager, int widgetID, 
			double traff, double allTraff, double mBytes) {
		RemoteViews widgetView = new RemoteViews(context.getPackageName(), R.layout.widget);
		widgetView.setTextViewText(R.id.tvInfoDigit, NumberFormat.getInstance().format(mBytes - traff) + ", MB");
		widgetView.setTextViewText(R.id.tvUsed, context.getResources().getString(R.string.widget_used) 
				+ NumberFormat.getInstance().format(traff) + ", MB");
		widgetView.setTextViewText(R.id.tvAll, context.getResources().getString(R.string.widget_all)
