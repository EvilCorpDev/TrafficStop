...
public class TrafficControl extends Activity implements
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (runActivity == 1) {
			this.overridePendingTransition(R.anim.slite_in_right,
					R.anim.slide_out_right);
		} else if (runActivity == 2) {
			this.overridePendingTransition(R.anim.slite_in_left,
		...
		startService(new Intent(this, TrafficConrolService.class)
		.putExtra("mBytes", mBytes)
		.putExtra("timeOff", sdPref.getBoolean("pref_Time_on", true))
		.putExtra("hour", sdPref.getInt("pref_Time.hour", 0))
		.putExtra("minute",
				sdPref.getInt("pref_Time.minute", 0)));

		br = new BroadcastReceiver() {
			public void onReceive(android.content.Context context, Intent intent) {
				updateText(intent.getDoubleExtra("traff", 0),
		...
		if (!sdPref.getBoolean("never", false)
				&& !sdPref.getBoolean("later", false)) {
			showDialog(DIALOG);
	...
	private void initialize() {
		...
		tvLimit.setText(getResources().getString(R.string.limit_status)
				+ mBytes + ", Mb");
	...
	public void updateText(double traff, double allTraff) {
		tvMain.setText(getResources().getString(R.string.data_day) + " "
				+ NumberFormat.getInstance().format(traff) + ", Mb"
				+ getResources().getString(R.string.all_data) + " "
				+ NumberFormat.getInstance().format(allTraff) + ", Mb");
		tvProgress.setText(getResources().getString(R.string.download) + " "
				+ NumberFormat.getInstance().format((traff / mBytes) * 100)
				+ ", %");
	...
	public void stopServ(View v) {
		...
		Toast.makeText(this,
				getResources().getString(R.string.stopped_service),
				Toast.LENGTH_SHORT).show();
	}

	public void startServ(View v) {
		startService(new Intent(this, TrafficConrolService.class).putExtra(
				"mBytes", mBytes));
		Toast.makeText(this,
				getResources().getString(R.string.started_service),
				Toast.LENGTH_SHORT).show();
	}
	...
	public void onSharedPreferenceChanged(SharedPreferences sd, String key) {
		try {
			if (key.equals("pref_Mbytes") || key.equals("pref_foreground")) {
				...
				startService(new Intent(this, TrafficConrolService.class)
						.putExtra("mBytes", mBytes));
			} else if (key.equals("pref_Time_on")
					|| key.equals("pref_Time.hour")
					|| key.equals("pref_Time.minute")) {
				stopServ(btnStop);
				startService(new Intent(this, TrafficConrolService.class)
						.putExtra("mBytes", mBytes)
						.putExtra("timeOff", sdPref.getBoolean("pref_Time_on", true))
